...
### Part c. shortest and longest Collatz sequence

```{r}
start_values <- 100:500
sequence_lengths <- numeric(length(start_values))

for (i in seq_along(start_values)) {
  seq_info <- collatzSequence(start_values[i])
  sequence_lengths[i] <- seq_info$length
}
...
# In case of ties, select the lowest starting value
shortest_start <- start_values[which(sequence_lengths == min_length)[1]]
longest_start  <- start_values[which(sequence_lengths == max_length)[1]]

# Compute the actual sequences for reporting
shortest_sequence <- collatzSequence(shortest_start)$sequence
longest_sequence  <- collatzSequence(longest_start)$sequence
