...
df_list <- split(df, df$country)

#' Standardize predictors within a country

#' @param data A single-country data frame.
#' @return gpa_std, forum_posts_std, quiz_attempts_std.
#' @export
standardize_country <- function(data) {
  data %>%
    mutate(
      gpa_std          = scale(prior_gpa),
      forum_posts_std  = scale(forum_posts),
      quiz_attempts_std = scale(quiz_attempts)
    )
}

df_list_std <- lapply(df_list, standardize_country)

#' Fit mixed-effects logistic regression for one country
#'
#' @param dat A standardized single-country data frame.
#' @return A glmer model object or a try-error object if fitting fails.
#' @export
fit_model <- function(dat) {
  try(
    glmer(
      completion ~ gpa_std + forum_posts_std + quiz_attempts_std + (1 | device_type),
      data = dat,
      family = binomial,
...
system_time <- system.time({
  results_parallel <- mclapply(df_list_std, fit_model, mc.cores = num_cores)
})

system_time

#' Extract forum_posts_std coefficient from model
#'
#' @param model A glmer model or try-error object.
#' @return Numeric coefficient for forum_posts_std, or NA.
#' @export
get_forum_coef <- function(model) {
  # If model failed, return NA
  ...
  as.numeric(coefs["forum_posts_std"])  # Return numeric only
}

coef_forum_parallel <- sapply(results_parallel, get_forum_coef, USE.NAMES = TRUE)
coef_forum_parallel
```

I tried running parallel::mclapply() to run models simultaneously to reduce the time which I efficiently did; however, there is some error in my code that I couldn't figure out so I keep getting NAs.
